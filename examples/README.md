...
| `acc90` | ACC'90 benchmark, spring-mass; QFT Toolbox manual example 5 (margin 2.25); Nataraj and Kubal, IJRNC 17 (2007), ex. 4.1 (p. 262-263) | `e/(s²(s²+0.02s+2e))`; `e ∈ [0.5,2]` | stability `1.75` and nothing else | 0.1 0.98 0.99 1 2 5 7 8.5 10 15 20 100 | `1.139e7(s+0.0751)(s+0.3488)(s+0.3868)/((s+7.2019)(s+39.7899)(s+95.8659)(s+96.2539))` — 3 zeros, 4 poles | yes |
...
| `aircraft` | Nandkishor 2006, ex. 5.2 (p. 71-73), over Thompson and Nwokah 1994 | `k(1+s/a)/(s(1+s/b)(1+2ζs/ωn+s²/ωn²))`; `k ∈ [0.2,2]`, `a ∈ [0.5,0.75]`, `b ∈ [1,10]`, `ωn ∈ [5,6]`, `ζ ∈ [0.8,0.9]` | stability **6 dB**; tracking `T_U = (1+s/0.35)/((1+s/0.5)(1+s/3))`, `T_L = 1/((1+s)²(1+s/5))` | 0.01 0.05 0.1 0.2 1 5 10 | `42.36(s+3.88)(s+4.46)(s+6.37)/((s+0.66)(s+25.02)(s+58.68))` — 3 zeros, 3 poles | yes |
...
| `jet-g12` | Tharewal 2005, ex. 6.2 (p. 92-96) | jet engine `43/(s³+a s²+b s)`; `a ∈ [0.004,0.005]`, `b ∈ [0.4,0.5]` | stability — the paper's `1.001` is replaced here by `1.2`; tracking `T_U = 5/(s+5)`, `T_L = 2460.375/((s+50.38)(s+13.5)(s+3.617))` | 0.001 0.008 0.06 0.5 | PID `(Kp + Ki/s + Kd s)/(s+10⁶)` | structure differs |
| `unstable` | Tharewal 2005, ex. 3.6 (p. 47-48) | unstable `k(s+a)/(s²−2.5)`; `k ∈ [1,10]`, `a ∈ [0.1,1]` | stability `2.1` and nothing else ("the only design spec considered") | 0.1 1 2 6 50 | `5.18` — a static gain | yes |
