...
private:
    ...
    struct NodeAnalysis {
        QVector<data_box *> datos;
        QVector<QPointF> boxMag;     //dB edges of the projected box
        QVector<QPointF> boxPhase;   //degree edges
        tools::flags_box flag = tools::feasible;
        qint32 mainFrequency = 0;    //largest ambiguous projected area
        bool anyFullPhaseWidth = false;
        ~NodeAnalysis();
    };

    inline bool analyse(Tripleta2 * node, NodeAnalysis & out);
    inline void improveNode(Tripleta2 * node, NodeAnalysis & analysis,
                            QVector<FeasibleThreshold> & thresholds);
    inline bool bestGainSearch(Tripleta2 * node, const NodeAnalysis & analysis);
    inline void feasibleCuts(Tripleta2 * node, const NodeAnalysis & analysis,
                             QVector<FeasibleThreshold> & thresholds, bool & improved);
    inline void infeasibleCuts(Tripleta2 * node, const NodeAnalysis & analysis,
                               bool & improved);

    inline FC::return_bisection2 bisect(Tripleta2 * node, const NodeAnalysis & analysis,
                                        const QVector<FeasibleThreshold> & thresholds);
    inline FC::return_bisection2 bisectAt(Tripleta2 * node, qint32 parameter, qreal point);
    inline qint32 widestByMeasure(Tripleta2 * node, qint32 mainFrequency, int measure);

    inline bool boxIsFeasibleAt(LtiSystem * box, qint32 freqIndex);
    inline bool boxIsFeasible(LtiSystem * box);
    inline void insertFeasibleBox(LtiSystem * box, Tripleta2 * parent);
