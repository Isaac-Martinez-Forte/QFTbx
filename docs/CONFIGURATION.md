...
| `mc.infeasible-magnitude`, `mc.infeasible-phase`, `mc.feasible-magnitude`, `mc.feasible-phase`, `mc.best-gain`, `mc.tree-bisection`, `mc.stages` | 1 | 0 or 1 | MC (thesis) and MC2: the strategies of chapter 4 of the thesis, one switch each. None changes the answer; they exist so that a benchmark plan can measure what each one buys |
