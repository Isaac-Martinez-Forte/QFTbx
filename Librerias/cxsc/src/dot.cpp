...
dotprecision::dotprecision(void) noexcept(false) : akku(new a_btyp[A_LENGTH]), err(0.0), k(0)
{
...
dotprecision::dotprecision(const dotprecision &from) noexcept(false)
                                 : akku(new a_btyp[A_LENGTH]), err(from.err), k(from.k)
{
   memcpy((void *)akku,(void *)from.akku,BUFFERSIZE);
}

dotprecision::dotprecision(const real &r) noexcept(false)
...
dotprecision & dotprecision::operator =(const dotprecision &from) noexcept(false)
{
...
dotprecision & dotprecision::operator =(const real &r) noexcept(false)
{
   real dummy(r);
...
dotprecision operator +(const dotprecision &a,const dotprecision &b) noexcept(false)
{
   dotprecision tmp(a); 
...
dotprecision operator +(const dotprecision &d,const real &r) noexcept(false)
{
   dotprecision erg(d);
...
dotprecision operator +(const real &r,const dotprecision &d) noexcept(false)
{
   dotprecision erg(d);
   d_radd(&(erg.akku),*((a_real *)&r));
   return erg;
...
dotprecision & operator +=(dotprecision &d,const real &r) noexcept(false)
{
   d_radd(&d.akku,*((a_real *)&r));
   return d;
}

dotprecision & operator +=(dotprecision &d,const dotprecision &d2) noexcept(false)
{
...
dotprecision operator -(const dotprecision &a) noexcept(false)
{
   dotprecision tmp(a);
   tmp.negdot();
   return tmp;
...
bool operator ==(const dotprecision &a,const dotprecision &b)  noexcept(false)  // Uebernommen aus dot.cpp "testequal"
{
   int res = true;
...
bool operator <=(const dotprecision &x,const dotprecision &y) noexcept(false)   // Uebernommen aus dot.cpp "testlessequal"
{
   int res = true, cont;
...
bool operator !=(const dotprecision &a,const dotprecision &b) noexcept(false) { return !(a==b); }
bool operator  <(const dotprecision &a,const dotprecision &b) noexcept(false) { return !(b<=a); }
bool operator  >(const dotprecision &a,const dotprecision &b) noexcept(false) { return !(a<=b); }
bool operator >=(const dotprecision &a,const dotprecision &b) noexcept(false) { return (b<=a);  }

bool operator ==(const real &r,const dotprecision &d) noexcept(false) { return dotprecision(r)==d; }
bool operator !=(const real &r,const dotprecision &d) noexcept(false) { return dotprecision(r)!=d; }
bool operator  <(const real &r,const dotprecision &d) noexcept(false) { return dotprecision(r)<d;  }  
bool operator  >(const real &r,const dotprecision &d) noexcept(false) { return dotprecision(r)>d;  }
bool operator <=(const real &r,const dotprecision &d) noexcept(false) { return dotprecision(r)<=d; }
...
bool operator ==(const dotprecision &d,const real &r) noexcept(false) { return d==dotprecision(r); }
bool operator !=(const dotprecision &d,const real &r) noexcept(false) { return d!=dotprecision(r); }
bool operator  <(const dotprecision &d,const real &r) noexcept(false) { return d<dotprecision(r);  }
bool operator  >(const dotprecision &d,const real &r) noexcept(false) { return d>dotprecision(r);  }
bool operator <=(const dotprecision &d,const real &r) noexcept(false) { return d<=dotprecision(r); }
...
bool operator  !(const dotprecision &a) noexcept(false)               { return sign(a)==0; }


void rnd (const dotprecision& d, real& r, rndtype type) noexcept(false) 
{
   if(type==RND_NEXT) {
...
void rnd (const dotprecision& d, real& l, real& u) noexcept(false) 
{
...
void rnd (const dotprecision& d, interval& x) noexcept(false) 
{
    real a,b;
    rnd(d,a,b);
    x = interval(a,b);
...
real rnd (const dotprecision& d, rndtype type) noexcept(false) 
{
    real r;
   if(type==RND_NEXT) {
      *((a_real *)(&r))=d_stan((Dotprecision)d.akku);
...
/*std::string  & operator << (std::string  & s, const dotprecision & d) noexcept(false) 
{
...
std::ostream & operator << (std::ostream & o, const dotprecision & d) noexcept(false) 
{
   std::string buffer;
   buffer << d;
   o << d;
...
std::istream & operator >> (std::istream & i,dotprecision & d) noexcept(false) 
{
...
*/
dotprecision & dotprecision::negdot(void) noexcept(false) 
{
   this->akku[A_SIGN] = 1 - this->akku[A_SIGN];
   return *this;
}

int sign(const dotprecision &a) noexcept(false) 
{
...
dotprecision abs(const dotprecision &a) noexcept(false) 
{
   if(sign(a)>=0)
      return a;
   dotprecision tmp(a);
...
dotprecision & accumulate (dotprecision& dot, const real& a, const real& b) noexcept(false) 
{
