...
lx_interval::lx_interval(const real& n, const string &s) noexcept(false)
// Constructor:
...
l_interval & l_interval::operator = (const lx_interval &a) noexcept(false)
{
    int exa;
    real p(expo(a)),exr;
...
interval & interval::operator = (const lx_interval &a) noexcept(false)
{
    l_interval al;
    interval z;
    al = a;
...
lx_interval::lx_interval(const lx_real& a, const lx_real& b) noexcept(false)
{
    lx_real a_(a),b_(b);
    l_real lr;
    l_interval h;
...
lx_interval operator + (const lx_interval &a, const lx_interval &b) noexcept(false)
{
...
std::string & operator >> (std::string &s, lx_interval &a) noexcept(false)
...
void operator >> (const std::string &s, lx_interval &a) noexcept(false)
{
...
void operator >> (const char *s, lx_interval &a) noexcept(false)
{
    std::string r(s);
    r >> a;
}
...
std::istream & operator >> (std::istream &s, lx_interval &a) noexcept(false)
...
std::ostream& operator << (std::ostream& s,const lx_interval& a) noexcept(false)
...
std::string & operator << (std::string &s, const lx_interval &a) noexcept(false)
...
// ----------------------------------------------------------------------------
	...
	lx_real::lx_real(const real& n, const string &s) noexcept(false)
// Constructor:
...
// (ex,li) is an approximation of  10^n*s;
{
	...
	std::string & operator >> (std::string &s, lx_real &a) noexcept(false)
...
// with:    10^(-4000)*2 ~ a;
{
	...
	void operator >> (const std::string &s, lx_real &a) noexcept(false)
{
   // Writes string s to variable a of type lx_real;
	std::string r(s);
	r >> a;
	...
	void operator >> (const char *s, lx_real &a) noexcept(false)
{
	std::string r(s);
	r >> a;
}	
...
// returns 1, if s contains char a or char b, otherwise 0;
{
	...
	std::istream & operator >> (std::istream &s, lx_real &a) noexcept(false)
...
bool operator == (const lx_real &a, const lx_real &b) noexcept(false)
{
	const real c1 = 1e20;
...
void scale_up(lx_real &a) noexcept(false)
...
void scale_down(lx_real &a) noexcept(false)
// scale_down skaliert a moeglichst weit nach unten, ohne
// dass dabei binaere Stellen verloren gehen.
// Der mathematische Wert von a bleibt dabei erhalten, d.h.
// 2^(a.ex)*a.li  hat vor und nach der Skalierung stets den
...
lx_real upper_bnd(const lx_real& x) noexcept(false)
...
lx_real lower_bnd(const lx_real& x) noexcept(false)
...
l_real & l_real::operator = (const lx_real& a) noexcept(false)
{
	int exar;
	real p(expo(a)), exr;
	l_real ar(lr_part(a));
...
real & real::operator = (const lx_real& a) noexcept(false)
{
	l_real lr;
	real x;
...
lx_interval sqrt(const lx_interval &a) noexcept(false)
{
	int stagsave = stagprec,
...
lx_interval sqr(const lx_interval &x) noexcept(false)
{
	int stagsave = stagprec,
 stagmax = 39;
 if (stagprec>stagmax) stagprec = stagmax;
...
lx_interval Lnp1(const lx_interval &x) noexcept(false)
...
lx_interval Ln_(const lx_interval &x) noexcept(false) 
{
	lx_interval T(0.0);
...
lx_interval ln(const lx_interval &x) noexcept(false) 
{
	int stagsave = stagprec,
 stagmax = 39;
 if (stagprec>stagmax) stagprec = stagmax;
...
lx_interval log2(const lx_interval &x) noexcept(false)
{
	int stagsave = stagprec,
 stagmax = 39;
 if (stagprec>stagmax) stagprec = stagmax;
...
lx_interval log10(const lx_interval &x) noexcept(false)
{
	int stagsave = stagprec,
 stagmax = 39;
 if (stagprec>stagmax) stagprec = stagmax;
...
lx_interval Lnp1_(const lx_interval &x) noexcept(false)
{
	const real r = 1e-7;
	lx_interval res;
...
lx_interval Power_(const lx_interval &x, const real &n) noexcept(false)
...
lx_interval power(const lx_interval &x, const real &n) noexcept(false)
{
	int stagsave = stagprec,
 stagmax = 38;
 if ( !(Is_Integer(n)) ) 
...
lx_interval Exp_(const lx_interval &x) noexcept(false)
{
	int ex,n,N;
...
lx_interval exp(const lx_interval &x) noexcept(false)
{
	int stagsave = stagprec,
 stagmax = 40;
...
lx_interval exp2(const lx_interval &x) noexcept(false)
{
	int stagsave = stagprec,
 stagmax = 40;
 if (stagprec>stagmax) stagprec = stagmax;
...
lx_interval exp10(const lx_interval &x) noexcept(false)
{
	int stagsave = stagprec,
 stagmax = 40;
 if (stagprec>stagmax) stagprec = stagmax;
...
lx_interval pow(const lx_interval &x, const lx_interval &e) noexcept(false)
...
lx_interval xp1_pow_y(const lx_interval &x, const lx_interval &y) noexcept(false)
{
	int stagsave = stagprec,
 stagmax = 40;
 if (stagprec>stagmax) stagprec = stagmax;
...
lx_interval EXPm1(const lx_interval &x) noexcept(false)
//  e^x - 1;
//  Only for internal use and for
//  not too wide intervals! 
{
...
// Rounding to the next integer number z, with z <= x;
 ...
 lx_interval sin(const lx_interval &x) noexcept(false)
// Inclusion of sin(x)
// Blomquist, 11.03.2008;
 ...
 lx_interval sin_n(const lx_interval &x, const real& n) noexcept(false)
...
//    if (n%2 != 0) res = -res;
 ...
 lx_interval cos(const lx_interval &x) noexcept(false)
// Inclusion of cos(x)
 {
	 int stagsave = stagprec,
  stagmax = 39;
 ...
 lx_interval cos_n(const lx_interval &x, const real& n) noexcept(false)
...
//    if (n%2 == 0) res = -res;
 ...
 lx_interval tan(const lx_interval& x) noexcept(false) 
 {
	 lx_interval c,y;
 ...
 lx_interval cot(const lx_interval & x) noexcept(false) 
 {
	 lx_interval s,y;
 ...
 lx_interval sqrt1px2(const lx_interval &x) noexcept(false) 
...
// Only for the internal use in the Atan_-function.
 ...
 lx_interval Atan_(const lx_interval &x) noexcept(false) 
...
// Only for the internal use in atan(x)!
 ...
 lx_interval atan(const lx_interval &x) noexcept(false)
 {
	 int stagsave = stagprec,
  stagmax = 39;
...
// ---------------------- sqrt(1-x^2) -----------------------------------
 ...
 lx_interval Sqrt1mx2_(const lx_interval &x) noexcept(false) 
...
// Blomquist, 08.04.2008;
 ...
 lx_interval sqrt1mx2(const lx_interval &x) noexcept(false)
 {
	 int stagsave = stagprec,
  stagmax = 39;
...
// ------------------- sqrt(x^2-1) ------------------------------------
 ...
 lx_interval sqrtx2m1(const lx_interval &x) noexcept(false) 
...
// ------------------- asin(x) ----------------------------------------
 ...
 lx_interval Asin_(const lx_interval& x) noexcept(false)
...
// Only for the internal use in asin(x)!
 ...
 lx_interval asin(const lx_interval &x) noexcept(false)
 {
	 int stagsave = stagprec,
  stagmax = 39;
...
// ------------------- acos(x) ----------------------------------------
 ...
 lx_interval Acos_(const lx_interval& x) noexcept(false)
...
// Blomquist, 24.04.2008;
 ...
 lx_interval acos(const lx_interval &x) noexcept(false)
// Blomquist, 24.04.2008;
 {
	 int stagsave = stagprec,
  stagmax = 39;
...
// ------------------- acot(x) ----------------------------------------
 ...
 lx_interval Acot_(const lx_interval& x) noexcept(false)
...
// Blomquist, 25.04.2008; 
 ...
 lx_interval acot(const lx_interval &x) noexcept(false)
// Blomquist, 24.04.2008;
 {
	 int stagsave = stagprec,
  stagmax = 39;
...
// -------------------------- sinh(...) --------------------------------------
 ...
 lx_interval Sinh_(const lx_interval &x) noexcept(false)
// Inclusion of sinh(x) for sufficiently tight intervals x
 ...
 lx_interval sinh(const lx_interval &x) noexcept(false)
 {
	 int stagsave = stagprec,
  stagmax = 39;
...
// -------------------------- cosh(...) ------------------------------------
 ...
 lx_interval Cosh_(const lx_interval& x) noexcept(false)
...
// Blomquist, 26.04.2008; 
 ...
 lx_interval cosh(const lx_interval &x) noexcept(false)
 {
	 int stagsave = stagprec,
  stagmax = 39;
...
// -------------------------- tanh(...) ------------------------------------
 ...
 lx_interval Tanh_(const lx_interval& x) noexcept(false)
 {
// Blomquist, 27.04.2008;
 ...
 lx_interval tanh(const lx_interval &x) noexcept(false)
 {
	 int stagsave = stagprec,
  stagmax = 39;
...
// -------------------------- coth(...) ------------------------------------
 ...
 lx_interval Coth_(const lx_interval& x) noexcept(false)
// This function is only used for tight intervals x,
// with Inf(x) > 0;
// Blomquist, 27.04.2008;
 {
 ...
 lx_interval coth(const lx_interval &x) noexcept(false)
// Blomquist, 27.04.2008;
 {
	 int stagsave = stagprec,
  stagmax = 39;
...
// -------------------------- sqrt(1+x)-1 ------------------------------------
 ...
 lx_interval Sqrtp1m1_(const lx_interval& x) noexcept(false)
...
// Blomquist, 25.08.07;
 ...
 lx_interval sqrtp1m1(const lx_interval &x) noexcept(false)
 {
	 int stagsave = stagprec,
  stagmax = 30;
...
// -------------------------- asinh(x) ------------------------------------
 ...
 lx_interval Asinh_(const lx_interval &x) noexcept(false)
// Inclusion of asinh(x)
 ...
 lx_interval asinh(const lx_interval &x) noexcept(false)
 {
	 int stagsave = stagprec,
  stagmax = 39;
  if (stagprec>stagmax) 
...
// -------------------------- acosh(...) -------------------------------
 ...
 lx_interval acosh(const lx_interval &x) noexcept(false)
...
// ------------------------- acoshp1(...) -----------------------------
 ...
 lx_interval Acoshp1_(const lx_interval &x) noexcept(false)
// This function is only used for tight intervals x,
// with Inf(x) >= 0;
// Blomquist, 03.05.2008;
 ...
 lx_interval acoshp1(const lx_interval &x) noexcept(false)
...
// ------------------------ atanh(...) ----------------------------------
 ...
 lx_interval Atanh_(const lx_interval &x) noexcept(false)
...
// Blomquist, 03.05.2008;
 ...
 lx_interval atanh(const lx_interval &x) noexcept(false)
 {
	 int stagsave = stagprec,
  stagmax = 39;
  if (stagprec>stagmax) 
...
// ------------------------ atanh1m(...) ---------------------------------
 ...
 lx_interval Atanh1m_(const lx_interval &x) noexcept(false)
...
// Blomquist, 07.05.2008;
 ...
 lx_interval atanh1m(const lx_interval &x) noexcept(false)
// Blomquist, 07.05.2008;
 {
	 int stagsave = stagprec,
  stagmax = 39;
...
// ------------------------ atanhm1p(...) ------------------------------
 ...
 lx_interval atanhm1p(const lx_interval &x) noexcept(false)
// Inclusion of atanh(-1+x) , 0<x<2;
 {
	 int stagsave = stagprec,
  stagmax = 39;
...
// ------------------------ acoth(...) ---------------------------------
 ...
 lx_interval Acoth_(const lx_interval& x) noexcept(false)
...
//  Blomquist, 13.05.2008;
 ...
 lx_interval acoth(const lx_interval &x) noexcept(false)
// Blomquist, 13.05.2008;
 {
	 int stagsave = stagprec,
  stagmax = 39;
...
// ------------------------ acoth(1+x) ---------------------------------
 ...
 lx_interval Acothp1_(const lx_interval& x) noexcept(false)
//  arcoth(1+x), x>0; 
...
//  Blomquist, 14.05.2008;
 ...
 lx_interval acothp1(const lx_interval &x) noexcept(false)
// Blomquist, 14.05.2008;
 {
	 int stagsave = stagprec,
  stagmax = 39;
...
// ------------------------ acothm1m(...) ------------------------------
 ...
 lx_interval acothm1m(const lx_interval &x) noexcept(false)
...
// ------------------------ sqrt(x^2+y^2) ------------------------------
 ...
 lx_interval Sqrtx2y2_(const lx_interval &x, const lx_interval &y) noexcept(false)
...
// Blomquist, 15.05.2008;
 ...
 lx_interval sqrtx2y2(const lx_interval &x, const lx_interval &y) noexcept(false)
 {
	 int stagsave = stagprec,
  stagmax = 30,
  exyl,exxl;
...
// ---------------------- ln(sqrt(x^2+y^2)) ----------------------------
 ...
 lx_interval Ln_sqrtx2y2_(const lx_interval &x, const lx_interval &y) noexcept(false)
...
// Blomquist, 19.05.2008;
 ...
 lx_interval ln_sqrtx2y2(const lx_interval &x, const lx_interval &y) noexcept(false)
 {
	 int stagsave = stagprec,
  stagmax = 30;
  if (stagprec>stagmax) stagprec = stagmax;
...
// -------------------------- n-th root --------------------------------
 ...
 lx_interval Sqrt_(const lx_interval& x, int n) noexcept(false)
 ...
 lx_interval sqrt(const lx_interval &x, int n) noexcept(false)
 {
	 int stagsave = stagprec,
  stagmax = 39,
  k(n);
...
// ---------------- Interval constants in high accuracy --------------------
 ...
 lx_interval Pi_lx_interval() noexcept(false)
// Inclusion of Pi, Blomquist, 15.06.2007;
 ...
 lx_interval Ln2_lx_interval() noexcept(false)
// Inclusion of ln(2), Blomquist, 15.06.2007;
 ...
 lx_interval Ln10_lx_interval() noexcept(false)
// Inclusion of ln(10), Blomquist, 15.06.2007;
 ...
 lx_interval Pir_lx_interval() noexcept(false)
// Inclusion of 1/Pi, Blomquist, 15.06.2007;
 ...
 lx_interval SqrtPi_lx_interval() noexcept(false)
// Inclusion of sqrt(Pi), Blomquist, 15.06.2007;
 ...
 lx_interval Sqrt2Pi_lx_interval() noexcept(false)
// Inclusion of sqrt(2*Pi), Blomquist, 15.06.2007;
 ...
 lx_interval Sqrt2_lx_interval() noexcept(false)
// Inclusion of sqrt(2), Blomquist, 15.06.2007;
 ...
 lx_interval Sqrt3_lx_interval() noexcept(false)
// Inclusion of sqrt(3), Blomquist, 15.06.2007;
 ...
 lx_interval Ln2r_lx_interval() noexcept(false)
// Inclusion of 1/ln(2), Blomquist, 15.06.2007;
 ...
 lx_interval Pid3_lx_interval() noexcept(false)
// Inclusion of Pi/3, Blomquist, 15.06.2007;
 ...
 lx_interval SqrtPir_lx_interval() noexcept(false)
// Inclusion of 1/sqrt(Pi), Blomquist, 15.06.2007;
 ...
 lx_interval Sqrt2Pir_lx_interval() noexcept(false)
// Inclusion of 1/sqrt(2*Pi), Blomquist, 15.06.2007;
 ...
 lx_interval LnPi_lx_interval() noexcept(false)
// Inclusion of ln(Pi), Blomquist, 15.06.2007;
 ...
 lx_interval Ln2Pi_lx_interval() noexcept(false)
// Inclusion of ln(2*Pi), Blomquist, 15.06.2007;
 ...
 lx_interval E_lx_interval() noexcept(false)
// Inclusion of e, Blomquist, 15.06.2007;
 ...
 lx_interval EpPi_lx_interval() noexcept(false)
// Inclusion of e^Pi, Blomquist, 15.06.2007;
 ...
 lx_interval EulerGamma_lx_interval() noexcept(false)
// Inclusion of EulerGamma, Blomquist, 15.06.2007;
 ...
 lx_interval Catalan_lx_interval() noexcept(false)
// Inclusion of Catalan, Blomquist, 15.06.2007;
 ...
 lx_interval sqrt5_lx_interval() noexcept(false)
// Inclusion of sqrt(5), Blomquist, 15.06.2007;
 ...
 lx_interval sqrt7_lx_interval() noexcept(false)
// Inclusion of sqrt(7), Blomquist, 15.06.2007;
 ...
 lx_interval One_m_lx_interval() noexcept(false)
...
// Blomquist, 26.07.2007;
 ...
 lx_interval One_p_lx_interval() noexcept(false)
// Inclusion of 2^(-1023)*(2^(+1023) + 2^(-1074)) = 1+2^(-2097), 
// Blomquist, 27.07.2007;
 ...
 lx_interval Ln10r_lx_interval() noexcept(false)
...
// **************************************************************************
 ...
 lx_interval Pid4_lx_interval() noexcept(false)
// Inclusion of 1/ln(10), Blomquist, 27.11.2008;
 {
	 l_interval y;
	 int stagsave = stagprec,
...
// *************************************************************************
 ...
 lx_interval Pid2_lx_interval() noexcept(false)
// Inclusion of 1/ln(10), Blomquist, 27.11.2008;
 {
	 l_interval y;
	 int stagsave = stagprec,
...
// **************************************************************************
 ...
 lx_interval Pi2_lx_interval() noexcept(false)
...
// **************************************************************************
 ...
 lx_interval Pi2r_lx_interval() noexcept(false)
// Inclusion of 1/Pi, Blomquist, 15.06.2007;
 {
	 l_interval y;
	 int stagsave = stagprec,
...
// **************************************************************************
 ...
 lx_interval Pip2_lx_interval() noexcept(false)
// Inclusion of 1/Pi, Blomquist, 15.06.2007;
 {
	 l_interval y;
	 int stagsave = stagprec,
...
// **************************************************************************
 ...
 lx_interval Sqrt2r_lx_interval() noexcept(false)
...
// ************************************************************************
 ...
 lx_interval Sqrt3d2_lx_interval() noexcept(false)
// Inclusion of sqrt(3), Blomquist, 15.06.2007;
 {
	 l_interval y;
	 int stagsave = stagprec,
...
// **********************************************************************
 ...
 lx_interval Sqrt3r_lx_interval() noexcept(false)
...
// **********************************************************************
 ...
 lx_interval Er_lx_interval() noexcept(false)
// Inclusion of 1/sqrt(3), Blomquist, 27.11.2008;
 {
	 l_interval y;
	 int stagsave = stagprec,
...
// **********************************************************************
 ...
 lx_interval Ep2_lx_interval() noexcept(false)
...
// **********************************************************************
 ...
 lx_interval Ep2r_lx_interval() noexcept(false)
...
// **********************************************************************
 ...
 lx_interval Ep2Pi_lx_interval() noexcept(false)
...
// **********************************************************************
 ...
 lx_interval EpPid2_lx_interval() noexcept(false)
...
// **********************************************************************
 ...
 lx_interval EpPid4_lx_interval() noexcept(false)
...
// ------------------------------------------------------------------------------
 ...
 lx_real Pi_lx_real() noexcept(false)
 { return mid(Pi_lx_interval()); }
 lx_real Pip2_lx_real() noexcept(false)
 { return mid(Pip2_lx_interval()); }
 lx_real Pi2r_lx_real() noexcept(false)
 { return mid(Pi2r_lx_interval()); }
 lx_real Pi2_lx_real() noexcept(false)
 { return mid(Pi2_lx_interval()); }
 lx_real Pid4_lx_real() noexcept(false)
 { return mid(Pid4_lx_interval()); }
 lx_real Pid2_lx_real() noexcept(false)
 { return mid(Pid2_lx_interval()); }
 lx_real Ln2_lx_real() noexcept(false)
 { return mid(Ln2_lx_interval()); }
 lx_real Ln10_lx_real() noexcept(false)
 { return mid(Ln10_lx_interval()); }
 lx_real Ln10r_lx_real() noexcept(false)
 { return mid(Ln10r_lx_interval()); }
 lx_real Pir_lx_real() noexcept(false)
 { return mid(Pir_lx_interval()); }
 lx_real SqrtPi_lx_real() noexcept(false)
 { return mid(SqrtPi_lx_interval()); }
 lx_real Sqrt2Pi_lx_real() noexcept(false)
 { return mid(Sqrt2Pi_lx_interval()); }
 lx_real Sqrt2_lx_real() noexcept(false)
 { return mid(Sqrt2_lx_interval()); }
 lx_real Sqrt2r_lx_real() noexcept(false)
 { return mid(Sqrt2r_lx_interval()); }
 lx_real Sqrt3_lx_real() noexcept(false)
 { return mid(Sqrt3_lx_interval()); }
 lx_real Sqrt3r_lx_real() noexcept(false)
 { return mid(Sqrt3r_lx_interval()); }
 lx_real Sqrt3d2_lx_real() noexcept(false)
 { return mid(Sqrt3d2_lx_interval()); }
 lx_real Ln2r_lx_real() noexcept(false)
 { return mid(Ln2r_lx_interval()); }
 lx_real Pid3_lx_real() noexcept(false)
 { return mid(Pid3_lx_interval()); }
 lx_real SqrtPir_lx_real() noexcept(false)
 { return mid(SqrtPir_lx_interval()); }
 lx_real Sqrt2Pir_lx_real() noexcept(false)
 { return mid(Sqrt2Pir_lx_interval()); }
 lx_real LnPi_lx_real() noexcept(false)
 { return mid(LnPi_lx_interval()); }
 lx_real Ln2Pi_lx_real() noexcept(false)
 { return mid(Ln2Pi_lx_interval()); }
 lx_real E_lx_real() noexcept(false)
 { return mid(E_lx_interval()); }
 lx_real Ep2r_lx_real() noexcept(false)
 { return mid(Ep2r_lx_interval()); }
 lx_real Ep2_lx_real() noexcept(false)
 { return mid(Ep2_lx_interval()); }
 lx_real Er_lx_real() noexcept(false)
 { return mid(Er_lx_interval()); }
 lx_real EpPi_lx_real() noexcept(false)
 { return mid(EpPi_lx_interval()); }
 lx_real EpPid2_lx_real() noexcept(false)
 { return mid(EpPid2_lx_interval()); }
 lx_real EpPid4_lx_real() noexcept(false)
 { return mid(EpPid4_lx_interval()); }
 lx_real Ep2Pi_lx_real() noexcept(false)
 { return mid(Ep2Pi_lx_interval()); }
 lx_real EulerGamma_lx_real() noexcept(false)
 { return mid(EulerGamma_lx_interval()); }
 lx_real Catalan_lx_real() noexcept(false)
 { return mid(Catalan_lx_interval()); }
 lx_real sqrt5_lx_real() noexcept(false)
 { return mid(sqrt5_lx_interval()); }
 lx_real sqrt7_lx_real() noexcept(false)
 { return mid(sqrt7_lx_interval()); }
 lx_real One_m_lx_real() noexcept(false)
 { return mid(One_m_lx_interval()); }
 lx_real One_p_lx_real() noexcept(false)
...
// --------------------------------------------------------------------------
 ...
 lx_real sqrt(const lx_real& x) noexcept(false)
 { return mid(sqrt(lx_interval(x))); }
 lx_real sqr(const lx_real& x) noexcept(false)
 { return mid(sqr(lx_interval(x))); }
 lx_real ln(const lx_real& x) noexcept(false)
 { return mid(ln(lx_interval(x))); }
 lx_real log2(const lx_real& x) noexcept(false)
 { return mid(log2(lx_interval(x))); }
 lx_real log10(const lx_real& x) noexcept(false)
 { return mid(log10(lx_interval(x))); }
 lx_real lnp1(const lx_real& x) noexcept(false)
 { return mid(lnp1(lx_interval(x))); }
 lx_real exp(const lx_real& x) noexcept(false)
 { return mid(exp(lx_interval(x))); }
 lx_real exp2(const lx_real& x) noexcept(false)
 { return mid(exp2(lx_interval(x))); }
 lx_real exp10(const lx_real& x) noexcept(false)
 { return mid(exp10(lx_interval(x))); }
 lx_real expm1(const lx_real& x) noexcept(false)
 { return mid(expm1(lx_interval(x))); }
 lx_real power(const lx_real& x, const real& r) noexcept(false)
 { return mid(power(lx_interval(x),r)); }
 lx_real pow(const lx_real& x, const lx_real& y) noexcept(false)
 { return mid(pow(lx_interval(x),lx_interval(y))); }
 lx_real xp1_pow_y(const lx_real& x, const lx_real& y) noexcept(false)
 { return mid(xp1_pow_y(lx_interval(x),lx_interval(y))); }
 lx_real sin(const lx_real& x) noexcept(false)
 { return mid(sin(lx_interval(x))); }
 lx_real sin_n(const lx_real& x, const real& n) noexcept(false)
 { return mid(sin_n(lx_interval(x),n)); }
 lx_real cos(const lx_real& x) noexcept(false)
 { return mid(cos(lx_interval(x))); }
 lx_real cos_n(const lx_real& x, const real& n) noexcept(false)
 { return mid(cos_n(lx_interval(x),n)); }
 lx_real tan(const lx_real& x) noexcept(false)
 { return mid(tan(lx_interval(x))); }
 lx_real cot(const lx_real& x) noexcept(false)
 { return mid(cot(lx_interval(x))); }
 lx_real sqrt1px2(const lx_real& x) noexcept(false)
 { return mid(sqrt1px2(lx_interval(x))); }
 lx_real atan(const lx_real& x) noexcept(false)
 { return mid(atan(lx_interval(x))); }
 lx_real sqrt1mx2(const lx_real& x) noexcept(false)
 { return mid(sqrt1mx2(lx_interval(x))); }
 lx_real sqrtx2m1(const lx_real& x) noexcept(false)
 { return mid(sqrtx2m1(lx_interval(x))); }
 lx_real asin(const lx_real& x) noexcept(false)
 { return mid(asin(lx_interval(x))); }
 lx_real acos(const lx_real& x) noexcept(false)
 { return mid(acos(lx_interval(x))); }
 lx_real acot(const lx_real& x) noexcept(false)
 { return mid(acot(lx_interval(x))); }
 lx_real sinh(const lx_real& x) noexcept(false)
 { return mid(sinh(lx_interval(x))); }
 lx_real cosh(const lx_real& x) noexcept(false)
 { return mid(cosh(lx_interval(x))); }
 lx_real tanh(const lx_real& x) noexcept(false)
 { return mid(tanh(lx_interval(x))); }
 lx_real coth(const lx_real& x) noexcept(false)
 { return mid(coth(lx_interval(x))); }
 lx_real sqrtp1m1(const lx_real& x) noexcept(false)
 { return mid(sqrtp1m1(lx_interval(x))); }
 lx_real asinh(const lx_real& x) noexcept(false)
 { return mid(asinh(lx_interval(x))); }
 lx_real acosh(const lx_real& x) noexcept(false)
 { return mid(acosh(lx_interval(x))); }
 lx_real acoshp1(const lx_real& x) noexcept(false)
 { return mid(acoshp1(lx_interval(x))); }
 lx_real atanh(const lx_real& x) noexcept(false)
 { return mid(atanh(lx_interval(x))); }
 lx_real atanh1m(const lx_real& x) noexcept(false)
 { return mid(atanh1m(lx_interval(x))); }
 lx_real atanhm1p(const lx_real& x) noexcept(false)
 { return mid(atanhm1p(lx_interval(x))); }
 lx_real acoth(const lx_real& x) noexcept(false)
 { return mid(acoth(lx_interval(x))); }
 lx_real acothp1(const lx_real& x) noexcept(false)
 { return mid(acothp1(lx_interval(x))); }
 lx_real acothm1m(const lx_real& x) noexcept(false)
 { return mid(acothm1m(lx_interval(x))); }
 lx_real sqrtx2y2(const lx_real& x, const lx_real& y) noexcept(false)
 { return mid(sqrtx2y2(lx_interval(x),lx_interval(y))); }
 lx_real ln_sqrtx2y2(const lx_real& x, const lx_real& y) noexcept(false)
 { return mid(ln_sqrtx2y2(lx_interval(x),lx_interval(y))); }
 lx_real sqrt(const lx_real& x, int n) noexcept(false)
