...
template <class MV,class V> friend  MV &_mvvsectassign(MV &v,const V &rv)
	...
	cvector () noexcept(false);
	//! Constructor of class cvector
	explicit cvector(const int &i) noexcept(false);
#ifdef OLD_CXSC
	//! Constructor of class cvector
	explicit cvector(const class index &i) noexcept(false); // for backwards compatibility
#endif
	//! Constructor of class cvector
...
#endif
	//! Constructor of class cvector
	cvector(const cmatrix_subv &) noexcept(false);
	//! Constructor of class cvector
	explicit cvector(const complex& r) noexcept(false);
	//! Constructor of class cvector
...
#endif
	//! Constructor of class cvector
	cvector(const cvector_slice &rs) noexcept(false);
	//! Constructor of class cvector
	cvector(const cvector &v) noexcept(false);
	//! Constructor of class cvector
	...
	//! Constructor of class cvector
	explicit cvector(const real &) noexcept(false);
	//! Constructor of class cvector
	explicit cvector(const rvector_slice &rs) noexcept(false);
	//! Constructor of class cvector
	explicit cvector(const rvector &v) noexcept(false);
	//! Constructor of class cvector
...
#endif
	//! Constructor of class cvector
	explicit cvector(const rmatrix_subv &) noexcept(false);
	
	// complex
	//! Implementation of standard assigning operator
	cvector &operator =(const cvector &rv) noexcept(false);
	//! Implementation of standard assigning operator
	cvector &operator =(const cvector_slice &sl) noexcept(false);
	//! Implementation of standard assigning operator
	cvector &operator =(const scvector &rv);
	//! Implementation of standard assigning operator
	cvector &operator =(const scvector_slice &sl);
	//! Implementation of standard assigning operator
	cvector &operator =(const complex &r) noexcept(false);
	//! Implementation of standard assigning operator
...
#endif
	//! Implementation of standard assigning operator
	cvector &operator =(const cmatrix_subv &) noexcept(false);
	// Real
	//! Implementation of standard assigning operator
	cvector &operator =(const rvector &rv) noexcept(false);
	//! Implementation of standard assigning operator
	cvector &operator =(const rvector_slice &sl) noexcept(false);
	//! Implementation of standard assigning operator
	cvector &operator =(const srvector &rv);
	//! Implementation of standard assigning operator
	cvector &operator =(const srvector_slice &sl);
	//! Implementation of standard assigning operator
	cvector &operator =(const real &r) noexcept(false);
	//! Implementation of standard assigning operator
...
#endif
	//! Implementation of standard assigning operator
	cvector &operator =(const rmatrix_subv &) noexcept(false);
	...
	friend INLINE int Lb(const cvector &rv) noexcept(false) { return rv.l; }
	//! Returns the upper bound of the vector
	friend INLINE int Ub(const cvector &rv) noexcept(false) { return rv.u; }
	//! Returns the dimension of the vector
        friend INLINE int VecLen(const cvector &rv) noexcept(false) { return rv.size; }
	//! Sets the lower bound of the vector
	friend INLINE cvector & SetLb(cvector &rv, const int &l) noexcept(false) { rv.l=l; rv.u=l+rv.size-1; return rv;}
	//! Sets the upper bound of the vector
	friend INLINE cvector & SetUb(cvector &rv, const int &u) noexcept(false) { rv.u=u; rv.l=u-rv.size+1; return rv;}
	...
	INLINE cvector & operator ()() noexcept(false) { return *this; }
...
throw(ERROR__OP_WITH_WRONG_DIM<cvector_slice>); #else 	noexcept(false);
#endif
...
*/  // 4.10.00 S.W.
 ...
 template <class VS1,class VS2> friend 	 VS1 &_vsvssectassign(VS1 &sl1, const VS2 &sl2)
	...
	explicit INLINE cvector_slice(cvector &a, const int &lb, const int &ub) noexcept(false):dat(a.dat),l(a.l),u(a.u),size(ub-lb+1),start(lb),end(ub) { }
	//! Constructor of class cvector_slice
	explicit INLINE cvector_slice(cvector_slice &a, const int &lb, const int &ub) noexcept(false):dat(a.dat),l(a.l),u(a.u),size(ub-lb+1),start(lb),end(ub) { }
	public:
	//! Constructor of class cvector_slice
	INLINE cvector_slice(const cvector_slice &a) noexcept(false):dat(a.dat),l(a.l),u(a.u),size(a.size),start(a.start),end(a.end) { }
	public:
	// complex
	//! Implementation of standard assigning operator
...
#endif
	//! Implementation of standard assigning operator
	INLINE cvector_slice & operator =(const complex &r) noexcept(false);
	//! Implementation of standard assigning operator
...
#endif
	//! Implementation of standard assigning operator
	INLINE cvector_slice &operator =(const cmatrix_subv &) noexcept(false);
	// Real
	//! Implementation of standard assigning operator
...
#endif
	//! Implementation of standard assigning operator
	INLINE cvector_slice & operator =(const real &r) noexcept(false);
	//! Implementation of standard assigning operator
...
#endif
	//! Implementation of standard assigning operator
	INLINE cvector_slice &operator =(const rmatrix_subv &mv) noexcept(false);
 ...
 template <class V,class MS,class E> friend 	 E _vmscimult(const V &v,const MS &ms)
...
#endif
	//! Returns the lower bound of the vector
	friend INLINE int Lb(const cvector_slice &sl) noexcept(false) { return sl.start; }
	//! Returns the upper bound of the vector
	friend INLINE int Ub(const cvector_slice &sl) noexcept(false) { return sl.end; }
	//! Returns the dimension of the vector
        friend INLINE int VecLen(const cvector_slice &sl) noexcept(false) { return sl.end-sl.start+1; }
	//! Operator for accessing the single elements of the vector (read-only)
	INLINE complex & operator [](const int &i) const
#if(CXSC_INDEX_CHECK)
	noexcept(false);
	...
	//! Operator for accessing the whole vector
	INLINE cvector_slice & operator ()() noexcept(false) { return *this; }
	//! Operator for accessing a part of the vector
	INLINE cvector_slice operator ()(const int &i)
#if(CXSC_INDEX_CHECK)
	noexcept(false);
...
//======================== Vector Functions =============================
	...
	INLINE cvector _cvector(const complex &r) noexcept(false);
//	INLINE cvector _cvector(const cmatrix &m) noexcept(false);
//	INLINE cvector _cvector(const cmatrix_slice &sl) noexcept(false);
	//! Deprecated typecast, which only exist for the reason of compatibility with older versions of C-XSC
	INLINE cvector _cvector(const real &r) noexcept(false);
	//! Deprecated typecast, which only exist for the reason of compatibility with older versions of C-XSC
	INLINE cvector _cvector(const rvector_slice &rs) noexcept(false);
	//! Deprecated typecast, which only exist for the reason of compatibility with older versions of C-XSC
	INLINE cvector _cvector(const rvector &rs) noexcept(false);
//	INLINE cvector _cvector(const rmatrix &m) noexcept(false);
//	INLINE cvector _cvector(const rmatrix_slice &sl) noexcept(false);
	//! Deprecated typecast, which only exist for the reason of compatibility with older versions of C-XSC
	INLINE cvector _cvector(const rmatrix_subv &rs) noexcept(false);
	...
	INLINE cvector &SetRe(cvector &iv,const real &r) noexcept(false);
	//! Sets componentwise the imaginary parts of the cvector
	INLINE cvector &SetIm(cvector &iv,const real &r) noexcept(false);
	//! Sets componentwise the real parts of the cvector
	INLINE cvector_slice &SetRe(cvector_slice &iv,const real &r) noexcept(false);
	//! Sets componentwise the imaginary parts of the cvector
	INLINE cvector_slice &SetIm(cvector_slice &iv,const real &r) noexcept(false);

	//! Resizes the vector
	INLINE void Resize(cvector &rv) noexcept(false);
	//! Resizes the vector
	...
	INLINE cvector conj(const cvector &rv) noexcept(false);
	//! Returns the conjugated cvector
	INLINE cvector conj(const cvector_slice &sl) noexcept(false);
	
	//! Returns the absolute value of the vector
	INLINE rvector abs(const cvector &rv) noexcept(false);
	//! Returns the absolute value of the vector
	INLINE rvector abs(const cvector_slice &sl) noexcept(false);
	//! Returns the imaginary part of the vector
	INLINE rvector Im(const cvector &v) noexcept(false);
	//! Returns the imaginary part of the vector
	INLINE rvector Im(const cvector_slice &v) noexcept(false);
	//! Returns the real part of the cvector
	INLINE rvector Re(const cvector &v) noexcept(false);
	//! Returns the real part of the cvector
	INLINE rvector Re(const cvector_slice &v) noexcept(false);
	//! Implementation of standard negation operation
	INLINE bool operator !(const cvector &rv) noexcept(false);
	//! Implementation of standard negation operation
	INLINE bool operator !(const cvector_slice &sl) noexcept(false);
...
//----------------------------- complex ---------------------------
	...
	INLINE cvector operator *(const cvector &rv, const complex &s) noexcept(false);
	//! Implementation of multiplication operation
	INLINE cvector operator *(const cvector_slice &sl, const complex &s) noexcept(false);
	//! Implementation of multiplication operation
	INLINE cvector operator *(const complex &s, const cvector &rv) noexcept(false);
	//! Implementation of multiplication operation
	INLINE cvector operator *(const complex &s, const cvector_slice &sl) noexcept(false);
	//! Implementation of multiplication and allocation operation
	INLINE cvector &operator *=(cvector &rv,const complex &r) noexcept(false);
...
//---------------------------- Real --------------------------------------
	...
	//! Implementation of multiplication operation
	INLINE cvector operator *(const cvector &rv, const real &s) noexcept(false);
	//! Implementation of multiplication operation
	INLINE cvector operator *(const cvector_slice &sl, const real &s) noexcept(false);
	//! Implementation of multiplication operation
	INLINE cvector operator *(const real &s, const cvector &rv) noexcept(false);
	//! Implementation of multiplication operation
	INLINE cvector operator *(const real &s, const cvector_slice &sl) noexcept(false);
	//! Implementation of multiplication and allocation operation
	INLINE cvector &operator *=(cvector &rv,const real &r) noexcept(false);
	...
	//! Implementation of multiplication operation
	INLINE cvector operator *(const rvector &rv, const complex &s) noexcept(false);
	//! Implementation of multiplication operation
	INLINE cvector operator *(const rvector_slice &sl, const complex &s) noexcept(false);
	//! Implementation of multiplication operation
	INLINE cvector operator *(const complex &s, const rvector &rv) noexcept(false);
	//! Implementation of multiplication operation
	INLINE cvector operator *(const complex &s, const rvector_slice &sl) noexcept(false);
...
//======================= Vector / Vector ===============================
	...
	INLINE std::ostream &operator <<(std::ostream &s, const cvector &rv) noexcept(false);
	//! Implementation of standard output method
	INLINE std::ostream &operator <<(std::ostream &o, const cvector_slice &sl) noexcept(false);
	//! Implementation of standard input method
	INLINE std::istream &operator >>(std::istream &s, cvector &rv) noexcept(false);
	//! Implementation of standard input method
	INLINE std::istream &operator >>(std::istream &s, cvector_slice &rv) noexcept(false);
...
//----------------------- complex / complex ---------------------------
	...
	INLINE const cvector &operator +(const cvector &rv) noexcept(false);
	//! Implementation of positive sign operation
	INLINE cvector operator +(const cvector_slice &sl) noexcept(false);
	...
	INLINE cvector operator -(const cvector &rv) noexcept(false);
	//! Implementation of negative sign operation
	INLINE cvector operator -(const cvector_slice &sl) noexcept(false);
	...
	INLINE bool operator ==(const cvector &rv1, const cvector &rv2) noexcept(false);
	//! Implementation of standard equality operation
	INLINE bool operator ==(const cvector_slice &sl1, const cvector_slice &sl2) noexcept(false);
	//! Implementation of standard equality operation
	INLINE bool operator ==(const cvector_slice &sl, const cvector &rv) noexcept(false);
	//! Implementation of standard equality operation
	INLINE bool operator ==(const cvector &rv, const cvector_slice &sl) noexcept(false);
