...
template <class MV,class V> friend  V _mvre(const MV &mv) noexcept(false);
 ...
 template <class VS1,class VS2,class E> friend 	 E _vsvslimult(const VS1 & sl1, const VS2 &sl2)
	...
	INLINE ivector () noexcept(false);
	//! Constructor of class ivector
	explicit INLINE ivector(const int &i) noexcept(false);
#ifdef OLD_CXSC
	//! Constructor of class ivector
	explicit INLINE ivector(const class index &i) noexcept(false); // for backwards compatibility
#endif
	//! Constructor of class ivector
...
#endif
	//! Constructor of class ivector
	INLINE ivector(const imatrix_subv &) noexcept(false);
	//! Constructor of class ivector
	explicit INLINE ivector(const interval &) noexcept(false);
	//! Constructor of class ivector
//	explicit INLINE ivector(const imatrix &)
...
#endif
	//! Constructor of class ivector
	INLINE ivector(const ivector_slice &rs) noexcept(false);
	//! Constructor of class ivector
	INLINE ivector(const ivector &v) noexcept(false);
	//! Constructor of class ivector
	...
	// Real
	//! Constructor of class ivector
	explicit INLINE ivector(const real &) noexcept(false);
	//! Constructor of class ivector
	explicit INLINE ivector(const rvector_slice &rs) noexcept(false);
	//! Constructor of class ivector
	...
	//! Constructor of class ivector
	explicit INLINE ivector(const rvector &v) noexcept(false);
	//! Constructor of class ivector
...
#endif
	//! Constructor of class ivector
//	explicit INLINE ivector(const rmatrix_subv &) noexcept(false);
        explicit        ivector(const rmatrix_subv &) noexcept(false);	
	
	// Interval
	//! Implementation of standard assigning operator
	INLINE ivector &operator =(const ivector &rv) noexcept(false);
	//! Implementation of standard assigning operator
	INLINE ivector &operator =(const ivector_slice &sl) noexcept(false);
	//! Implementation of standard assigning operator
	...
	//! Implementation of standard assigning operator
	INLINE ivector &operator =(const interval &r) noexcept(false);
	//! Implementation of standard assigning operator
...
#endif
	//! Implementation of standard assigning operator
	INLINE ivector &operator =(const imatrix_subv &) noexcept(false);
	// Real
	//! Implementation of standard assigning operator
	INLINE ivector &operator =(const rvector &rv) noexcept(false);
	//! Implementation of standard assigning operator
	INLINE ivector &operator =(const rvector_slice &sl) noexcept(false);
	//! Implementation of standard assigning operator
	INLINE ivector &operator =(const real &r) noexcept(false);
	//! Implementation of standard assigning operator
...
#endif
	//! Implementation of standard assigning operator
	INLINE ivector &operator =(const rmatrix_subv &) noexcept(false);
	...
	friend INLINE int Lb(const ivector &rv) noexcept(false) { return rv.l; }
	//! Returns the upper bound of the vector
	friend INLINE int Ub(const ivector &rv) noexcept(false) { return rv.u; }
	//! Returns the dimension of the vector
        friend INLINE int VecLen(const ivector &rv) noexcept(false) { return rv.size; }
	//! Sets the lower bound of the vector
	friend INLINE ivector &SetLb(ivector &rv, const int &l) noexcept(false) { rv.l=l; rv.u=l+rv.size-1; return rv; }
	//! Sets the upper bound of the vector
	friend INLINE ivector &SetUb(ivector &rv, const int &u) noexcept(false) { rv.u=u; rv.l=u-rv.size+1; return rv; }
	...
	INLINE ivector & operator ()() noexcept(false) { return *this; }
...
template <class VS,class V,class E> friend 	 E _vsvlimult(const VS & sl, const V &rv)
	...
	explicit INLINE ivector_slice(ivector &a, const int &lb, const int &ub) noexcept(false):dat(a.dat),l(a.l),u(a.u),size(ub-lb+1),start(lb),end(ub) { }
	//! Constructor of class ivector_slice
	explicit INLINE ivector_slice(ivector_slice &a, const int &lb, const int &ub) noexcept(false):dat(a.dat),l(a.l),u(a.u),size(ub-lb+1),start(lb),end(ub) { }
	public: 
	//! Constructor of class ivector_slice
	INLINE ivector_slice(const ivector_slice &a) noexcept(false):dat(a.dat),l(a.l),u(a.u),size(a.size),start(a.start),end(a.end) { }
	public:
	// Interval
	//! Implementation of standard assigning operator
...
#endif
	//! Implementation of standard assigning operator
	INLINE ivector_slice & operator =(const interval &r) noexcept(false);
	//! Implementation of standard assigning operator
...
#endif
	//! Implementation of standard assigning operator
	INLINE ivector_slice &operator =(const imatrix_subv &) noexcept(false);
	// Real
	//! Implementation of standard assigning operator
...
#endif
	//! Implementation of standard assigning operator
	INLINE ivector_slice & operator =(const real &r) noexcept(false);
	//! Implementation of standard assigning operator
...
#endif
	//! Implementation of standard assigning operator
	INLINE ivector_slice &operator =(const rmatrix_subv &mv) noexcept(false);
...
#endif
	//! Returns the lower bound of the vector
	friend INLINE int Lb(const ivector_slice &sl) noexcept(false) { return sl.start; }
	//! Returns the upper bound of the vector
	friend INLINE int Ub(const ivector_slice &sl) noexcept(false) { return sl.end; }
	//! Returns the dimension of the vector
        friend INLINE int VecLen(const ivector_slice &sl) noexcept(false) { return sl.end-sl.start+1; }
	//! Operator for accessing the single elements of the vector (read-only)
	INLINE interval & operator [](const int &i) const
#if(CXSC_INDEX_CHECK)
	noexcept(false);
	...
	//! Operator for accessing the whole vector
	INLINE ivector_slice & operator ()() noexcept(false) { return *this; }
	//! Operator for accessing a part of the vector
	INLINE ivector_slice operator ()(const int &i)
#if(CXSC_INDEX_CHECK)
	noexcept(false);
...
//======================== Vector Functions =============================
	...
	INLINE ivector _ivector(const interval &r) noexcept(false);
//	INLINE ivector _ivector(const imatrix &m) noexcept(false);
//	INLINE ivector _ivector(const imatrix_slice &sl) noexcept(false);
	//! Deprecated typecast, which only exist for the reason of compatibility with older versions of C-XSC
	INLINE ivector _ivector(const real &r) noexcept(false);
	//! Deprecated typecast, which only exist for the reason of compatibility with older versions of C-XSC
	INLINE ivector _ivector(const rvector_slice &rs) noexcept(false);
	//! Deprecated typecast, which only exist for the reason of compatibility with older versions of C-XSC
	INLINE ivector _ivector(const rvector &rs) noexcept(false);
//	INLINE ivector _ivector(const rmatrix &m) noexcept(false);
//	INLINE ivector _ivector(const rmatrix_slice &sl) noexcept(false);
	//! Deprecated typecast, which only exist for the reason of compatibility with older versions of C-XSC
	INLINE ivector _ivector(const rmatrix_subv &rs) noexcept(false);
	...
	INLINE ivector &SetSup(ivector &iv,const real &r) noexcept(false);
	//! Returns the vector with the new given infimum value
	INLINE ivector &SetInf(ivector &iv,const real &r) noexcept(false);
	//! Returns the vector with the new unchecked given supremum value
	INLINE ivector &UncheckedSetSup(ivector &iv,const real &r) noexcept(false);
	//! Returns the vector with the new unchecked given infimum value
	INLINE ivector &SetUncheckedInf(ivector &iv,const real &r) noexcept(false);
	//! Returns the vector with the new given supremum value
	INLINE ivector_slice &SetSup(ivector_slice &iv,const real &r) noexcept(false);
	//! Returns the vector with the new given infimum value
	INLINE ivector_slice &SetInf(ivector_slice &iv,const real &r) noexcept(false);
	//! Returns the vector with the new unchecked given supremum value
	INLINE ivector_slice &UncheckedSetSup(ivector_slice &iv,const real &r) noexcept(false);
	//! Returns the vector with the new unchecked given infimum value
	INLINE ivector_slice &SetUncheckedInf(ivector_slice &iv,const real &r) noexcept(false);

	//! Resizes the vector
	INLINE void Resize(ivector &rv) noexcept(false);
	//! Resizes the vector
	...
	INLINE ivector abs(const ivector &rv) noexcept(false);
	//! Returns the absolute value of the vector
	INLINE ivector abs(const ivector_slice &sl) noexcept(false);
	...
	INLINE rvector diam(const ivector &v) noexcept(false);
        //! Returns the rounded diameter of the vector
	INLINE rvector diam(const ivector_slice &v) noexcept(false);
	//! Returns the rounded middle of the vector
	INLINE rvector mid(const ivector &v) noexcept(false);
	//! Returns the rounded middle of the vector
	INLINE rvector mid(const ivector_slice &v) noexcept(false);
	//! Returns the infimum of the vector
	INLINE rvector Inf(const ivector &v) noexcept(false);
	//! Returns the infimum of the vector
	INLINE rvector Inf(const ivector_slice &v) noexcept(false);
	//! Returns the supremum of the vector
	INLINE rvector Sup(const ivector &v) noexcept(false);
	//! Returns the supremum of the vector
	INLINE rvector Sup(const ivector_slice &v) noexcept(false);
	//! Implementation of standard negation operation
	INLINE bool operator !(const ivector &rv) noexcept(false);
	//! Implementation of standard negation operation
	INLINE bool operator !(const ivector_slice &sl) noexcept(false);
...
//----------------------------- Interval ---------------------------
	...
	INLINE ivector operator *(const ivector &rv, const interval &s) noexcept(false);
	//! Implementation of multiplication operation
	INLINE ivector operator *(const ivector_slice &sl, const interval &s) noexcept(false);
	//! Implementation of multiplication operation
	INLINE ivector operator *(const interval &s, const ivector &rv) noexcept(false);
	//! Implementation of multiplication operation
	INLINE ivector operator *(const interval &s, const ivector_slice &sl) noexcept(false);
	//! Implementation of multiplication and allocation operation
	INLINE ivector &operator *=(ivector &rv,const interval &r) noexcept(false);
...
//---------------------------- Real --------------------------------------
	...
	//! Implementation of multiplication operation
	INLINE ivector operator *(const ivector &rv, const real &s) noexcept(false);
	//! Implementation of multiplication operation
	INLINE ivector operator *(const ivector_slice &sl, const real &s) noexcept(false);
	//! Implementation of multiplication operation
	INLINE ivector operator *(const real &s, const ivector &rv) noexcept(false);
	//! Implementation of multiplication operation
	INLINE ivector operator *(const real &s, const ivector_slice &sl) noexcept(false);
	//! Implementation of multiplication and allocation operation
	INLINE ivector &operator *=(ivector &rv,const real &r) noexcept(false);
	...
	//! Implementation of multiplication operation
	INLINE ivector operator *(const rvector &rv, const interval &s) noexcept(false);
	//! Implementation of multiplication operation
	INLINE ivector operator *(const rvector_slice &sl, const interval &s) noexcept(false);
	//! Implementation of multiplication operation
	INLINE ivector operator *(const interval &s, const rvector &rv) noexcept(false);
	//! Implementation of multiplication operation
	INLINE ivector operator *(const interval &s, const rvector_slice &sl) noexcept(false);
...
//======================= Vector / Vector ===============================
	...
	INLINE std::ostream &operator <<(std::ostream &s, const ivector &rv) noexcept(false);
	//! Implementation of standard output method
	INLINE std::ostream &operator <<(std::ostream &o, const ivector_slice &sl) noexcept(false);
	//! Implementation of standard input method
	INLINE std::istream &operator >>(std::istream &s, ivector &rv) noexcept(false);
	//! Implementation of standard input method
	INLINE std::istream &operator >>(std::istream &s, ivector_slice &rv) noexcept(false);
...
//----------------------- Interval / Interval ---------------------------
	...
	INLINE const ivector &operator +(const ivector &rv) noexcept(false);
	//! Implementation of positive sign operation
	INLINE ivector operator +(const ivector_slice &sl) noexcept(false);
	...
	INLINE bool operator ==(const ivector &rv1, const ivector &rv2) noexcept(false);
	//! Implementation of standard equality operation
	INLINE bool operator ==(const ivector_slice &sl1, const ivector_slice &sl2) noexcept(false);
	//! Implementation of standard equality operation
	INLINE bool operator ==(const ivector_slice &sl, const ivector &rv) noexcept(false);
	//! Implementation of standard equality operation
	INLINE bool operator ==(const ivector &rv, const ivector_slice &sl) noexcept(false);
