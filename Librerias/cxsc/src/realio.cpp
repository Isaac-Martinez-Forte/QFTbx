...
std::string & operator<<(std::string &s, const real& a) noexcept(false)
{
#if _WIN32
...
std::ostream & operator <<(std::ostream &o,const real &a) noexcept(false)
{
   std::string s="";
   s << a;
   o << s;
...
std::string & operator>> (std::string & str, real& a) noexcept(false)
{
...
void operator >>(const char *a,real &b) noexcept(false)
{
   std::string c(a);
   c>>b;
}
void operator >>(const string &a,real &b) noexcept(false)
{
   std::string c(a);
   c>>b;
}
std::istream& operator>> (std::istream& s, real& a) noexcept(false)
{
   if (ioflags.isset(IOFlags::hex))
   {
      char inp[20];
