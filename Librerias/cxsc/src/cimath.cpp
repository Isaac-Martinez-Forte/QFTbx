...
cinterval exp(const cinterval& z) noexcept(false)
{
  interval
    A( exp( Re(z) ) ),
    B(      Im(z)   );
...
cinterval exp2(const cinterval& z) noexcept(false)
{
	return exp(z*Ln2_interval);
}

cinterval exp10(const cinterval& z) noexcept(false)
{
	return exp(z*Ln10_interval);
}

cinterval expm1(const cinterval& z) noexcept(false)
// exp(z) - 1;
...
cinterval cos(const cinterval& z) noexcept(false)
{
  interval
    A( Re(z) ),
    B( Im(z) );
...
cinterval sin(const cinterval& z) noexcept(false)
{
  interval
    A( Re(z) ),
    B( Im(z) );
...
cinterval cosh(const cinterval& z) noexcept(false)
{
  interval
    A( Re(z) ),
    B( Im(z) );
...
cinterval sinh(const cinterval& z) noexcept(false)
{
  interval
    A( Re(z) ),
    B( Im(z) );
...
cinterval tan( const cinterval& z ) noexcept(false) //----------------------- 040827 --
{
  interval
      rez = Re(z),   // rez = z.re(),
      imz = Im(z);   // imz = z.im();
...
cinterval cot( const cinterval& z ) noexcept(false)
...
//  tanh( z ) = transp( i * tan( transp( i * z ) )
//
cinterval tanh( const cinterval& z ) noexcept(false)
{
...
cinterval coth( const cinterval& z ) noexcept(false)
{ // coth( z ) = i * cot( i * z );
...
//
//
interval Atan(const interval& y, const interval& x) noexcept(false)
...
interval Atan(const interval& y, const real& x) noexcept(false)
// Calculating an inclusion of atan(y/x) with x<>0.0;
...
//
interval Arg( const cinterval& z ) noexcept(false)
{
  real 
    srez = Sup( Re(z) ),
    irez = Inf( Re(z) ), 
...
//
interval arg_inclmon( const cinterval& z ) noexcept(false)
{
...
//  axis from below;
//
cinterval Ln( const cinterval& z ) noexcept(false)
{ // Blomquist;
  real
    srez = Sup( Re(z) ),
    simz = Sup( Im(z) ),
...
cinterval lnp1(const cinterval& z) noexcept(false)
{  // ln(1+z);
	// Calculates nearly optimal inclusions for not too wide intervals z.
   // Blomquist, 03.12.2008;
	const real c1 = 1.0;
...
cinterval log2( const cinterval& z ) noexcept(false)
{
	return Ln(z) / Ln2_interval;
}

cinterval log10( const cinterval& z ) noexcept(false)
{
...
cinterval sqrt( const cinterval& z ) noexcept(false) // -------------------------------
//
...
cinterval sqrtp1m1(const cinterval& z) noexcept(false)
// sqrt(1+z)-1;
// Calculates nearly optimal inclusions for not too wide intervals z.
...
cinterval sqrt1px2(const cinterval& z) noexcept(false)
// sqrt(1+z^2);
...
cinterval sqrtx2m1(const cinterval& z) noexcept(false)
// sqrt(z^2-1);
// Calculates nearly optimal inclusions for not too wide intervals z.
...
cinterval sqrt1mx2(const cinterval& z) noexcept(false)
// sqrt(1-z^2);
// Calculates nearly optimal inclusions for not too wide intervals z.
// Blomquist, 04.12.2008;
{
...
cinterval sqrt( const cinterval& z, int n ) noexcept(false) // ----- 040915 --
//
...
*/
cinterval power_fast( const cinterval& z, int n ) noexcept(false)
{
  if( n == 0 )
    return cinterval( ONE_INTERVAL() );
  else if( n == 1 )
...
*/
cinterval power( const cinterval& z, int n ) noexcept(false) //---- 040720 --
//
//  Power function for integer powers with optimal (save roundoff) accuracy.
//
{
...
cinterval pow( const cinterval& z, const interval& p ) noexcept(false)
...
cinterval pow( const cinterval& z, const cinterval& p ) noexcept(false)
{
    return exp( p*Ln(z) );
}
...
//
std::list<cinterval> pow_all( const cinterval& z, const interval& p ) noexcept(false)
{
  interval abs_z = abs( z );
...
cinterval asin( const cinterval& z ) noexcept(false) //---------------------- 040730 --
{
...
//-- acos ----------------------------------------------------------- 040730 --
//
cinterval acos( const cinterval& z ) noexcept(false)  //--------------------- 040730 --
{
    const real gr = 6.355804e307; // upper bound for abs(rez),abs(imz)
    interval 
	rez = Re(z),
...
//-- asinh ---------------------------------------------------------- 040730 --
//
cinterval asinh( const cinterval& z ) noexcept(false)
//
//  asinh( Z ) = i * asin( -i * z )
//
{
...
//-- acosh ---------------------------------------------------------- 040908 --
//
cinterval acosh( const cinterval& z ) noexcept(false)
//
//  acosh( z ) = i * acos( z ) = +/- i * ( pi / 2 - asin( z ) )
//
{
...
cinterval atan( const cinterval& z ) noexcept(false) //----- 040912 --
{
  interval
    rez = Re(z),
    imz = Im(z);
...
//
cinterval acot( const cinterval& z ) noexcept(false)
{
  interval
    rez = Re(z),
    imz = Im(z);
...
//-- atanh ---------------------------------------------------------- 040912 --
//
cinterval atanh( const cinterval& z ) noexcept(false)
//
//  atanh( z ) = - i * atan( i * z )
//
{
...
//-- acoth ---------------------------------------------------------- 040912 --
//
cinterval acoth( const cinterval& z ) noexcept(false)
//
//  acoth( z ) = i * acot( i * z )
//
{
...
cinterval sqr(const cinterval& z) noexcept(false)
