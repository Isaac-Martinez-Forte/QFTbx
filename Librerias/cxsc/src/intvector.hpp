...
template <class V,class MV2,class S> friend  V &_vmvassign(V &v,const MV2 &rv) noexcept(false);
 template <class MV,class S,class E> friend 	 E _mvsdiv(const MV &rv, const S &s) noexcept(false);
	...
	INLINE intvector () noexcept(false);
#ifdef OLD_CXSC
	//! Constructor of class intvector
	explicit INLINE intvector(const class index &i) noexcept(false); // for backwards compatibility
#endif
	//! Constructor of class intvector
...
#endif
	//! Constructor of class intvector
	INLINE intvector(const intmatrix_subv &) noexcept(false);
	//! Constructor of class intvector
	explicit INLINE intvector(const int &) noexcept(false);
	//! Constructor of class intvector
...
#endif
	//! Constructor of class intvector
	INLINE intvector(const intvector_slice &rs) noexcept(false);
	//! Constructor of class intvector
	INLINE intvector(const intvector &v) noexcept(false);
	//! Implementation of standard assigning operator
	INLINE intvector &operator =(const intvector &rv) noexcept(false);
	//! Implementation of standard assigning operator
	INLINE intvector &operator =(const intvector_slice &sl) noexcept(false);
	//! Implementation of standard assigning operator
	INLINE intvector &operator =(const int &r) noexcept(false);
	//! Implementation of standard assigning operator
...
#endif
	//! Implementation of standard assigning operator
	INLINE intvector &operator =(const intmatrix_subv &) noexcept(false);
	...
	friend INLINE int Lb(const intvector &rv) noexcept(false) { return rv.l; }
	//! Returns the upper bound of the vector
	friend INLINE int Ub(const intvector &rv) noexcept(false) { return rv.u; }
	//! Returns the dimension of the vector
        friend INLINE int VecLen(const intvector &rv) noexcept(false) { return rv.size; }
	//! Sets the lower bound of the vector
	friend INLINE intvector &SetLb(intvector &rv, const int &l) noexcept(false) { rv.l=l; rv.u=l+rv.size-1; return rv; }
	//! Sets the upper bound of the vector
	friend INLINE intvector &SetUb(intvector &rv, const int &u) noexcept(false) { rv.u=u; rv.l=u-rv.size+1; return rv; }
	...
	INLINE intvector & operator ()() noexcept(false) { return *this; }
...
//------------------------- Templates -------------------------------------------
 ...
 template <class VS1,class VS2,class E> friend 	 E _vsvsmult(const VS1 & sl1, const VS2 &sl2)
	...
	explicit INLINE intvector_slice(intvector &a, const int &lb, const int &ub) noexcept(false):dat(a.dat),l(a.l),u(a.u),size(ub-lb+1),start(lb),end(ub) { }
	//! Constructor of class intvector_slice
	explicit INLINE intvector_slice(intvector_slice &a, const int &lb, const int &ub) noexcept(false):dat(a.dat),l(a.l),u(a.u),size(ub-lb+1),start(lb),end(ub) { }
	public: 
	//! Constructor of class intvector_slice
	INLINE intvector_slice(const intvector_slice &a) noexcept(false):dat(a.dat),l(a.l),u(a.u),size(a.size),start(a.start),end(a.end) { }
	public:
	//! Constructor of class intvector_slice
...
#endif
	//! Implementation of standard assigning operator
	INLINE intvector_slice & operator =(const int &r) noexcept(false);
	//! Implementation of standard assigning operator
...
#endif
	//! Implementation of standard assigning operator
	INLINE intvector_slice &operator =(const intmatrix_subv &) noexcept(false);

	//--------------------- Standardfunktionen ------------------------------

	//! Returns the lower bound of the vector
	friend INLINE int Lb(const intvector_slice &sl) noexcept(false) { return sl.start; }
	//! Returns the upper bound of the vector
	friend INLINE int Ub(const intvector_slice &sl) noexcept(false) { return sl.end; }
	//! Returns the dimension of the vector
        friend INLINE int VecLen(const intvector_slice &sl) noexcept(false) { return sl.end-sl.start+1; }
	//! Operator for accessing the single elements of the vector
	INLINE int & operator [](const int &i)
#if(CXSC_INDEX_CHECK)
	noexcept(false);
...
#endif
	//! Operator for accessing the whole vector
	INLINE intvector_slice & operator ()() noexcept(false) { return *this; }
	//! Operator for accessing a part of the vector
	INLINE intvector_slice operator ()(const int &i)
#if(CXSC_INDEX_CHECK)
	noexcept(false);
...
//======================== Vector Functions =============================
	...
	INLINE intvector _intvector(const int &r) noexcept(false); 
...
//	INLINE intvector _intvector(const intmatrix_slice &sl) noexcept(false);
	...
	//! Resizes the vector
	INLINE void Resize(intvector &rv) noexcept(false);
	//! Resizes the vector
	...
	INLINE intvector abs(const intvector &rv) noexcept(false);
	//! Returns the absolute value of the vector
	INLINE intvector abs(const intvector_slice &sl) noexcept(false);
	//! Implementation of standard negation operation
	INLINE bool operator !(const intvector &rv) noexcept(false);
	//! Implementation of standard negation operation
	INLINE bool operator !(const intvector_slice &sl) noexcept(false);

//======================= Vector / Scalar ===============================

	//! Implementation of multiplication operation
	INLINE intvector operator *(const intvector &rv, const int &s) noexcept(false);
	//! Implementation of multiplication operation
	INLINE intvector operator *(const intvector_slice &sl, const int &s) noexcept(false);
	//! Implementation of multiplication operation
	INLINE intvector operator *(const int &s, const intvector &rv) noexcept(false);
	//! Implementation of multiplication operation
	INLINE intvector operator *(const int &s, const intvector_slice &sl) noexcept(false);
	//! Implementation of multiplication and allocation operation
	INLINE intvector &operator *=(intvector &rv,const int &r) noexcept(false);
...
//======================= Vector / Vector ===============================
	...
	INLINE const intvector &operator +(const intvector &rv) noexcept(false);
	//! Implementation of positive sign operation
	INLINE intvector operator +(const intvector_slice &sl) noexcept(false);
	...
	INLINE bool operator ==(const intvector &rv1, const intvector &rv2) noexcept(false);
	//! Implementation of standard equality operation
	INLINE bool operator ==(const intvector_slice &sl1, const intvector_slice &sl2) noexcept(false);
	//! Implementation of standard equality operation
	INLINE bool operator ==(const intvector_slice &sl, const intvector &rv) noexcept(false);
	//! Implementation of standard equality operation
	INLINE bool operator ==(const intvector &rv, const intvector_slice &sl) noexcept(false);
	...
	INLINE std::ostream &operator <<(std::ostream &s, const intvector &rv) noexcept(false);
	//! Implementation of standard output method
	INLINE std::ostream &operator <<(std::ostream &o, const intvector_slice &sl) noexcept(false);
	//! Implementation of standard input method
	INLINE std::istream &operator >>(std::istream &s, intvector &rv) noexcept(false);
	//! Implementation of standard input method
	INLINE std::istream &operator >>(std::istream &s, intvector_slice &rv) noexcept(false);
